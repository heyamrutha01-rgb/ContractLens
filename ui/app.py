...
from agent.analyzer import analyze_contract
...
if st.session_state.page == "Add Contract":
    ...
    if st.session_state.analysis_state == "idle":
        ...
        if analyze:
            ...
            if primary_file is None:
                ...
                with st.spinner("Analyzing your contract with Gemini..."):

                    try:

                        contract_text = extract_text_from_pdf(primary_file)
                        ...
                        st.warning("Gemini is temporarily unavailable. Loading the last verified ContractLens analysis for demo mode.")

                        st.session_state.contract_analysis = {
                            "contract_info": {
                                "parties": ["Alpha Code Studios", "Beta Retail Inc."],
                                "contract_type": "Fast-Track Service Agreement",
                                "effective_date": "October 1, 2026",
                                "expiration_date": "December 31, 2026",
                                "renewal_terms": "Not specified",
                                "termination_conditions": "Either party may cancel with 14 days written notice. Client pays only for completed milestones."
                            },
                            "financial_terms": {
                                "payment_terms": "Invoices must be paid within 10 days of milestone completion.",
                                "fees": "Phase 1: ,000 USD; Phase 2: 0,000 USD; Phase 3: ,000 USD (Total: 0,000 USD).",
                                "penalties": "Late payments incur a 1.5% weekly penalty."
                            },
                            "obligations": [
                                {"party": "Alpha Code Studios", "obligation": "Deliver Phase 1 UI/UX Design Mockups", "deadline": "October 15, 2026"},
                                {"party": "Alpha Code Studios", "obligation": "Deliver Phase 2 Beta Software Release", "deadline": "November 15, 2026"},
                                {"party": "Alpha Code Studios", "obligation": "Deliver Phase 3 Final Launch & Handover", "deadline": "December 15, 2026"},
                                {"party": "Beta Retail Inc.", "obligation": "Approve or request changes to deliverables", "deadline": "Within 3 business days of receipt"},
                                {"party": "Alpha Code Studios", "obligation": "Fix critical system bugs during active project phases", "deadline": "Within 24 hours"},
                                {"party": "Beta Retail Inc.", "obligation": "Pay milestone invoices", "deadline": "Within 10 days of milestone completion"}
                            ],
                            "review_items": [
                                {"clause": "2.1 Late payments incur a 1.5% weekly penalty.", "reason": "The late-payment penalty should receive human legal review."},
                                {"clause": "2.2 Client must approve or request changes within 3 business days.", "reason": "The agreement does not define whether silence results in deemed acceptance."},
                                {"clause": "2.3 Provider must fix critical system bugs within 24 hours.", "reason": "Critical system bug and the 24-hour window are not fully defined."},
                                {"clause": "2.4 Either party may cancel with 14 days written notice.", "reason": "Provider may have uncompensated work-in-progress if termination occurs mid-phase."}
                            ]
                        }
    ...
    # --------------------------------------------------------
        ...
        parties_text = " ↔ ".join(parties) if parties else "Not specified"
        ...
        st.html(
            ...
            <div class="analysis-card">
                ...
                <div class="analysis-step">
                    <strong>{info.get("contract_type", "Contract")}</strong>
                </div>
                ...
                <div class="analysis-step">
                    {len(review_items)} clauses require human review
                </div>
...
# ============================================================
    ...
    analysis = st.session_state.get("contract_analysis", {})
    raw_obligations = analysis.get("obligations", [])

    obligations = []
    for item in raw_obligations:
        obligations.append({
            "obligation": item.get("obligation", "Not specified"),
            "party": item.get("party", "Not specified"),
            "timing": item.get("deadline", "Not specified"),
            "status": "Ongoing" if item.get("deadline") == "Ongoing throughout the contract term (January 1, 2026 to December 31, 2028)" else "Upcoming",
            "contract": "Uploaded Contract",
            "section": "AI-extracted obligation",
...
# ============================================================
    ...
    # --------------------------------------------------------
    # --------------------------------------------------------
    # TIMELINE DATA FROM CONTRACT ANALYSIS
    # --------------------------------------------------------
    ...
    for item in raw_obligations:
        ...
        timeline_events.append({
            "date": deadline,
            "title": item.get("obligation", "Not specified"),
            "party": item.get("party", "Not specified"),
            "contract": info.get("contract_type", "Uploaded Contract"),
            "status": "Upcoming",
            "section": "AI-extracted obligation",
            "page": "Source contract",
        })

    expiration = info.get("expiration_date", "Not specified")

    if expiration != "Not specified":
        timeline_events.append({
            "date": expiration,
            "title": "Contract expiration",
            "party": " / ".join(info.get("parties", [])),
            "contract": info.get("contract_type", "Uploaded Contract"),
            "status": "Upcoming",
...
# ============================================================
    ...
    info = analysis.get("contract_info", {})
    parties = info.get("parties", [])
    raw_review_items = analysis.get("review_items", [])

    review_items = []

    for item in raw_review_items:
        review_items.append({
            "title": item.get("clause", "Clause requiring review"),
            "contract": info.get("contract_type", "Uploaded Contract"),
            "section": "AI-identified clause",
            "page": "Source contract",
            "reason": item.get("reason", "Human review recommended."),
            "source_text": item.get("clause", "Not specified"),
        })
